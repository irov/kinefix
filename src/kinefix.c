...
static kf_bool_t __kf_ratio_magnitude_to_fixed( uint64_t numerator, kf_bool_t numerator_negative, uint64_t denominator, kf_bool_t denominator_negative, kf_fixed_t * result )
{
    ...
    {
        *result = (kf_fixed_t)quotient;
    }
    else if( quotient == (UINT64_C(1) << 31) )
    {
        *result = INT32_MIN;
    }
    else
...
KF_FORCE_INLINE kf_bool_t __kf_fixed_madd_q32_wide( kf_fixed_t base, kf_fixed_t value, __kf_q32_t scalar, int64_t * result_q48 )
{
    ...
    if( value_magnitude != 0 && scalar_magnitude > UINT64_MAX / value_magnitude )
    {
        return KF_FALSE;
    }
...
kf_fixed_t kf_fixed_from_ratio( int64_t numerator, int64_t denominator )
{
    ...
    if( __kf_ratio_q32_to_fixed( numerator, denominator, &result ) == KF_TRUE )
    {
        return result;
    }
...
{
    return (float)value / (float)KF_FIXED_SCALE;
}
...
kf_fixed_t kf_fixed_div( kf_fixed_t left, kf_fixed_t right )
{
    ...
    if( __kf_fixed_try_div( left, right, &result ) == KF_TRUE )
    {
        return result;
    }
...
kf_fixed_t kf_fixed_wrap_angle( kf_fixed_t angle )
{
    ...
    if( angle > KF_FIXED_PI )
    {
        angle = kf_fixed_sub( angle, KF_FIXED_TWO_PI );
    }
    if( angle < -KF_FIXED_PI )
    {
        angle = kf_fixed_add( angle, KF_FIXED_TWO_PI );
    }
    return angle;
}
...
kf_sangle16_t kf_sangle16_from_fixed_radians( kf_fixed_t radians )
{
    ...
    if( wrapped <= (kf_angle16_t)INT16_MAX )
    {
        return (kf_sangle16_t)wrapped;
    }
...
kf_sangle16_t kf_sangle16_add_clamped( kf_sangle16_t angle, kf_sangle16_t delta, kf_sangle16_t minimum, kf_sangle16_t maximum )
{
    ...
    if( result < (int32_t)minimum )
    {
        result = minimum;
    }
    if( result > (int32_t)maximum )
    {
        result = maximum;
    }
...
void kf_angle16_sin_cos( kf_angle16_t angle, kf_fixed_t * sine, kf_fixed_t * cosine )
{
    if( sine != NULL )
    {
        *sine = __kf_angle16_sine( angle );
    }
    if( cosine != NULL )
    {
        *cosine = __kf_angle16_sine( (kf_angle16_t)(angle + KF_ANGLE16_QUARTER) );
    }
}
...
{
    return kf_vec3_dot( value, value );
}
...
kf_fixed_t kf_segment_point_distance_squared( kf_vec3_t start, kf_vec3_t end, kf_vec3_t point )
{
    ...
    if( length_squared_q32 == 0 || projection_q32 <= 0 )
    {
        return __kf_narrow_q32( point_distance_q32 );
    }
...
static kf_bool_t __kf_sweep_point_bounds( kf_vec3_t origin, kf_vec3_t displacement, const kf_fixed_t minimum[3], const kf_fixed_t maximum[3], uint32_t object_id, kf_hit_t * hit )
{
    ...
    for( axis = 0; axis != 3; ++axis )
    {
        ...
        if( axis_exit_q32 < exit_q32 )
        {
            exit_q32 = axis_exit_q32;
        }
        if( entry_q32 > exit_q32 )
        {
            return KF_FALSE;
        }
    }

    if( exit_q32 <= 0 || entry_q32 > unit_q32 )
    {
        return KF_FALSE;
    }
    if( hit != NULL )
...
kf_bool_t kf_overlap_sphere_capsule( const kf_sphere_t * sphere, const kf_capsule_t * capsule )
{
    ...
    if( __kf_vec3_dot_q32( relative, relative, &distance_squared ) == KF_FALSE )
    {
        return KF_FALSE;
    }
...
kf_bool_t kf_raycast_aabb( const kf_ray_t * ray, const kf_aabb_t * box, kf_hit_t * hit )
{
    ...
    kf_bool_t result;
    if( ray == NULL || box == NULL || ray->maximum_distance <= 0 )
    {
        return KF_FALSE;
    }
...
static kf_bool_t __kf_append_quadratic_root( uint64_t left, kf_bool_t left_negative, uint64_t right, kf_bool_t right_negative, uint64_t denominator, kf_bool_t denominator_negative, __kf_q32_t roots[2], size_t * count )
{
    ...
    if( __kf_ratio_magnitude_to_q32( numerator, numerator_negative, denominator, denominator_negative, roots + *count ) == KF_FALSE )
    {
        return KF_FALSE;
    }
    ++*count;
...
static size_t __kf_quadratic_roots_q32( __kf_q32_t a, __kf_q32_t b, __kf_q32_t c, __kf_q32_t roots[2] )
{
    ...
    if( a == 0 )
    {
        ...
        if( __kf_ratio_magnitude_to_q32( c_magnitude, (kf_bool_t)(c_negative == KF_FALSE && c != 0), b_magnitude, b_negative, roots ) == KF_FALSE )
        {
            return 0;
        }
        return 1;
    ...
    if( __kf_shift_left_two_u128( __kf_multiply_u64( a_magnitude, c_magnitude ), &four_ac ) == KF_FALSE )
    {
        return 0;
    }
    if( a_negative != c_negative )
    {
        if( __kf_add_u128( discriminant, four_ac, &discriminant ) == KF_FALSE )
        {
            return 0;
        }
    }
    ...
    {
        if( __kf_compare_u128( discriminant, four_ac ) < 0 )
        {
            return 0;
        }
    ...
    if( a_magnitude > UINT64_MAX / UINT64_C(2) )
    {
        return 0;
    }
...
kf_bool_t kf_sweep_sphere_capsule( const kf_sphere_t * sphere, kf_vec3_t displacement, const kf_capsule_t * capsule, uint32_t object_id, kf_hit_t * hit )
{
    ...
    if( __kf_vec3_dot_q32( displacement, displacement, &displacement_length_squared_q32 ) == KF_FALSE )
    {
        return KF_FALSE;
    }
    if( displacement_length_squared_q32 == 0 )
    {
        if( kf_overlap_sphere_capsule( sphere, capsule ) == KF_FALSE )
        {
            return KF_FALSE;
        }
        if( hit != NULL )
    ...
    else
    {
        ...
        for( index = 0; index != 2; ++index )
        {
            ...
            if( root_count != 0 )
            {
                __kf_consider_capsule_root( roots[0], KF_FALSE, 0, 0, 0, &found, &selected );
            }
        }
    }
    ...
    if( hit != NULL )
    {
        ...
        *hit = (kf_hit_t){0};
        ...
        {
            hit->normal = kf_vec3_mul( kf_vec3_normalize( displacement ), -KF_FIXED_SCALE );
        }
...
kf_bool_t kf_raycast_capsule( const kf_ray_t * ray, const kf_capsule_t * capsule, uint32_t object_id, kf_hit_t * hit )
{
    ...
    kf_vec3_t displacement;
    kf_sphere_t point;
    if( ray == NULL || ray->maximum_distance <= 0 || __kf_capsule_valid( capsule ) == KF_FALSE )
    {
        return KF_FALSE;
    }
    direction_length = kf_vec3_length( ray->direction );
...
kf_bool_t kf_world_find_step_top( const kf_aabb_t * boxes, size_t count, kf_vec3_t candidate, kf_fixed_t half_width, kf_fixed_t height, kf_fixed_t current_y, kf_fixed_t maximum_step, kf_fixed_t * top )
{
    ...
    for( index = 0; index != count; ++index )
    {
        ...
        if( box_top <= current_y || kf_fixed_sub( box_top, current_y ) > maximum_step )
        {
            continue;
        }
...
kf_bool_t kf_sweep_sphere_aabb( kf_vec3_t start, kf_vec3_t end, kf_fixed_t radius, const kf_aabb_t * box, kf_fixed_t * hit_time )
{
    ...
    if( result == KF_TRUE && hit_time != NULL )
    {
...
static kf_bool_t __kf_character_try_step( kf_character_body_t * body, const kf_character_config_t * config, const kf_aabb_t * boxes, size_t count, kf_vec3_t displacement, const kf_hit_t * blocking_hit )
{
    ...
    if( step == NULL || body->grounded == KF_FALSE )
    {
        return KF_FALSE;
    }
...
void kf_character_step( kf_character_body_t * body, const kf_character_config_t * config, const kf_aabb_t * boxes, size_t count, kf_character_result_t * result )
{
    ...
    *result = (kf_character_result_t){0};
    if( body == NULL || config == NULL || __kf_capsule_valid( &(kf_capsule_t){body->position, config->radius, config->height} ) == KF_FALSE )
    {
        return;
    }
